...
from src.utils.torch.evaluation import (
    visualize_ae_model_performance,
...
def ae_train_val_test_loop(
    ...
    for i in range(num_epochs):
        ...
        for phase in ["train", "val"]:
            epoch_statistics = process_ae_epoch(
...
def clf_train_val_test_loop(
    ...
    for i in range(num_epochs):
        ...
        # Iterate over training and validation phase
        for phase in ["train", "val"]:
            epoch_statistics = process_clf_epoch(
                domain_config=domain_config, phase=phase, device=device, epoch=i
            )
            ...
            if "accuracy" in epoch_statistics:
                logging.debug(
                    "Accuracy for {} domain: {:.8f}".format(
        ...
        if i % save_freq == 0:
            ...
            # visualize_clf_model_performance(output_dir=checkpoint_dir, domain_config=domain_config,
            #                                dataset_types=["train", "val"], device=device)
    ...
    if "test" in domain_config.data_loader_dict:
        ...
        # visualize_clf_model_performance(output_dir=test_dir, domain_config=domain_config,
        #                                dataset_types=["train", "val", "test"], device=device)
...
def process_clf_epoch(
    ...
    for index, samples in enumerate(
        ...
        correct += batch_statistics["correct"]
    ...
    epoch_statistics = {"total_loss": total_loss, "accuracy": accuracy}
    return epoch_statistics
...
def train_classifier(
    ...
    total_loss = domain_model_config.loss_function
