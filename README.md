...
### Step-by-step guide (approx. 30 minutes)
...
1. You are asked to select the conda environment used to run the demo. If this is the first time you run the demo, please simply hit enter. This setups a new conda environment called ``image2reg_demo`` that will contain the correct python version and all additional packages required for our inference example to run. If this is the second this is the first time you run the demo on the system, you can use the previosuly created ``image2reg_demo`` environment by typing in: ``image2reg_demo`` when the prompt appears.
