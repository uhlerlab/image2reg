...
Help()
{
   ...
   echo " We here provide a demo application that runs our pipeline one of the following overexpression conditions: BRAF, JUN, RAF1, SMAD4 or SREBF1 or alternatively for a user-defined imaging data set." | fold -sw 80
   echo 
   ...
   echo "If new is given inference is performed for a new image data set for which the raw chromatin images need to be provided in image2reg/test_data/images/raw/plate and the corresponding nuclear masks in images2reg/test_data/images/unet_masks/plate" | fold -sw 80
   echo
...
then
    target=$2
    shift 2
#elif [ "$1" = "--image_dir" -o "$1" = "-idir" ]
#then
#    image_dir=$2
#    shift 2
#elif [ "$1" = "--mask_dir" -o "$1" = "-mdir" ]
#then
#    mask_dir=$2
#    shift 2
elif [ "$1" = "--random" -o "$1" = "-r" ]
